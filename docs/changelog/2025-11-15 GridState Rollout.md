...
- Added `migrations/0001_grid_state.py` helper to convert legacy `state.json` files once per world; backups land beside the migrated file as `state.json.backup`, and every state now carries `_migration_version` metadata enforced by `core/world.py`.
