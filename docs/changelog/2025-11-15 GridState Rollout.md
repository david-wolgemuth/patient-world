...
- `sim.py` gained explicit `init-grid` (bootstrap deterministic grids) and `intervene` (global or cell-level edits) commands, plus the existing `tick`/`forecast` subcommands now operate on `GridState`.
